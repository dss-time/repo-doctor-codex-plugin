...
- 按[完整用户手册](docs/USER_MANUAL.zh-CN.md)安装和调用；不确定仓库任务该用什么时，从 `$repo-doctor-router` 开始。
- 在 [Skill 完整目录](docs/SKILL_CATALOG.zh-CN.md)查看全部 active Skill。若搜不到，核对所选插件/ZIP、重新构建或安装正确平台产物，并使用 canonical slug。
- 用 `npm run doctor` 或 `npm run doctor -- --json` 执行只读安装与同步自检。
- 更新时拉取可信版本并重新构建/安装目标平台产物；卸载时通过宿主移除已安装插件或 Skill，不要为了停用安装而删除 canonical `packs/`。
