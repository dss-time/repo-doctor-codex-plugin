...
2. Derive two to five realistic positive requests, explicit negative requests, and adjacent Skills. Ask only when a blocking choice would change scope, permissions, public/private placement, or the number of Skills.
...
6. Plan the minimum files. Keep core procedure in canonical instructions; add only referenced first-level `references/`, useful `assets/`, or deterministic tested `scripts/`. Never create empty resource directories or Skill-local README/CHANGELOG files.
...
8. Replace placeholders with bilingual metadata, imperative workflows, output contracts, examples, and tests. Put what/when/not-when semantics in descriptions; do not hard-code platform invocation syntax into canonical instructions.
...
10. Run the repository's deterministic quality checker, validation, activation tests, maintainer-tool tests, and build. Inspect every targeted output and repeat the build to verify stable hashes. Fix failures without hiding unknowns.
...
3. Positive triggers, counterexamples, and adjacent Skills
...
7. Known limitations and unperformed release actions
...
2. 推导 2–5 个真实正向请求、明确反例和相邻 Skill。只有会改变范围、权限、公开/私有归属或 Skill 数量的阻塞选择才询问。
...
6. 规划最少文件。核心流程保留在 canonical instructions；只增加被引用的一级 `references/`、有用 `assets/` 或有测试的确定性 `scripts/`。不得创建空资源目录或 Skill 内 README/CHANGELOG。
...
8. 用双语元数据、命令式流程、输出契约、示例和测试替换占位内容；description 写清做什么、何时触发、何时不触发；canonical 正文不写死平台调用语法。
9. 更新 Pack 索引、相关插件同步配置、UI 元数据、必要 manifests/marketplace、activation cases 和仓库文档，由 adapters 生成平台副本。
10. 运行确定性质量检查、validate、activation、维护工具测试和 build；抽查所有目标产物并重复构建比较哈希。修复失败，不隐藏未知项。
