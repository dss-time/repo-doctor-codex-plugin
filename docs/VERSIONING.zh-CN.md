...
当前候选版本是 **0.2.0**。它目前只是 Release Candidate：仓库中的版本记录本身不代表已经创建 commit、tag、GitHub Release，也不代表已经执行 npm 或插件市场发布。
...
Repo Doctor Skills 仍处于 1.0 之前。项目 0.2.0 候选版本在 0.1.0 内容基线之上增加了向后兼容的 Pack、Skill、平台产物和用户工作流，因此采用 minor 升级。1.0.0 必须是明确的产品决策。
...
项目历史和 SemVer 规划以 0.1.0 作为内容发布基线，同时原样保留已有 `v0.0.1` tag，不删除、不移动、不重建。只有在获得独立发布授权后，0.2.0 才应使用 `v0.2.0` 发布。
